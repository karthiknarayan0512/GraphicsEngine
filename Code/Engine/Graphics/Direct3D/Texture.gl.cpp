	...
	namespace Graphics
	{
		void Texture::LoadTexture(const char *i_texturePath)
		{
			...
			// Open the texture file
			{
				...
				fileHandle = CreateFile(i_texturePath, desiredAccess, otherProgramsCanStillReadTheFile,
			...
			// Iterate through each MIP map level and fill in the OpenGL texture
			{
				...
				for (uint32_t mipMapLevel = 0; mipMapLevel < ddsHeader->mipMapCount; ++mipMapLevel)
				{
					...
					if (errorCode == GL_NO_ERROR)
					{
						...
						currentWidth = 1 > (currentWidth / 2) ? 1 : (currentWidth / 2);
						currentHeight = 1 > (currentHeight / 2) ? 1 : (currentHeight / 2);
					}
					else
					{
						wereThereErrors = true;
		...
		void Texture::SetTexture(size_t i_textureUnit)
		{
			...
			glUniform1i(m_SamplerID, i_textureUnit);
			assert(glGetError() == GL_NO_ERROR);
		}
	}
}