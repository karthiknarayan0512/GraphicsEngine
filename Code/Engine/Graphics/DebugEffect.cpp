...
namespace eae6320
{
	namespace Graphics
	{
		bool DebugEffect::CreateEffect(const char *i_shaderBinaryFile)
		{
		...
		bool DebugEffect::LoadVertexShader(const char *i_vertexShaderFile)
		{
		...
		bool DebugEffect::LoadFragmentShader(const char *i_fragmentShaderfile)
		{
			// Load the source code from file
		...
		void DebugEffect::SetTransforms(Camera &i_Camera)
		{
			IDirect3DDevice9* direct3DDevice = Context::getDirect3DDevice();
			...
			HRESULT result = m_vertexShaderConstantTable->SetMatrixTranspose(direct3DDevice, m_WorldToViewTransform,
